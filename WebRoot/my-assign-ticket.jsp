...
ResultSet rs= DBUtilDaoImp.getConnection().createStatement().executeQuery("select * from createticket where Login_name='"+session.getAttribute("username").toString()+"' ");
		...
		while(rs.next()){
			...
			ticketForm.setPriority(rs.getString("priority"));
		...
		<display:column title="Status">  
                  <strong><%//=statusConta.getStagtus()%></strong>  
                        <select name="combo" id="Approvedstatus" >  
                            ...
                            <option value="NotApproved" >Working Progress</option>  
                            <option value="Checking" >Handled</option>  
                        </select>  </display:column>
		
</display:table>
<input type="button" value="EDIT" onclick="edits();">
