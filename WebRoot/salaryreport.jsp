...
<jsp:directive.page import="com.callcenter.formbean.salary"/>
...
ResultSet rs= DBUtilDaoImp.getConnection().createStatement().executeQuery("select * from salary ");

		while(rs.next()){
			salary  salaryform=new salary();
			salaryform.setBasic(rs.getString("basic"));
			salaryform.setTa(rs.getString("ta"));
			salaryform.setDa(rs.getString("da"));
			salaryform.setHra(rs.getString("hra"));
			salaryform.setTotal(rs.getString("total"));
			salaryform.setEid(rs.getInt("eid"));
			
			arrayList.add(salaryform);
			}
	} catch (Exception e) {
		e.printStackTrace();

	}
	session.setAttribute("salaryform", arrayList);
	System.out.println(arrayList.size());

 %>

<display:table name="sessionScope.salaryform" pagesize="10" export="true" decorator="" pagesize="5">
	<display:column property="eid" title="Empid" sortable="true" sortable="true"/>
	<display:column property="basic" title="Basic" sortable="true"/>
		<display:column property="ta" title="TA" sortable="true" />
		<display:column property="da" title="DA" sortable="true"/>
		<display:column property="hra" title="HRA" sortable="true"/>
